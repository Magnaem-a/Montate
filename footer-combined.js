...
<script>
  ...
  const restoreEnglish=()=>{
    try{
      ...
      setTimeout(()=>{
        const s2=window.google?.translate?.TranslateService?.getInstance();
        if(s2?.restore) s2.restore();
      },100);
      setTimeout(()=>{
        const s3=window.google?.translate?.TranslateService?.getInstance();
        if(s3?.restore) s3.restore();
      },300);
  ...
  async function applyLang(code){
    ...
    if(target==='en'){ 
      setGT(''); location.hash='';
      restoreEnglish();
    }else{ storeOriginalTexts(); setGT(`/en/${target}`); location.hash=`#googtrans(en|${target})`; }
    if(window.updateDatePlaceholders) window.updateDatePlaceholders();
    ...
    let isApplyingLang=false;
    const trigger=()=>{ 
      if(isApplyingLang) return;
      isApplyingLang=true;
      fire(combo); 
      ['change','input','blur'].forEach(t=>combo.dispatchEvent(new Event(t,{bubbles:true,cancelable:true}))); 
      setTimeout(()=>{isApplyingLang=false;},100);
    };
    if(target==='en'){
      ...
      if(combo.value===''||combo.value==='en'){ 
        combo.value='en'; 
        trigger(); 
        setTimeout(()=>{ combo.value=target; trigger(); },100); 
      }else{ 
        combo.value=target; 
        trigger(); 
      }
    ...
    getCombo().then(c=>{
      ...
      c.addEventListener('change',()=>{
        ...
        if(v==='en'){ 
          ...
          if(window.updateDatePlaceholders) window.updateDatePlaceholders();
          if(window.updateFlatpickrLocale) window.updateFlatpickrLocale();
        }else{ storeOriginalTexts(); setGT(`/en/${v}`); location.hash=`#googtrans(en|${v})`; }
        if(window.updateDatePlaceholders) window.updateDatePlaceholders();
        setTimeout(()=>{ protect(); applyCustomWordTranslations(v); if(v!=='en') restoreTranslationCase(); if(window.updateDatePlaceholders) window.updateDatePlaceholders(); isManual=false; },150);
      });
      c._msBound=true;
    });
